...
function checkAnswer(userGuess) {
    ...
    if(userGuess==answer){
        ...
        const currentAttemptAmount = previousAttemptAmount + 1;
        sessionStorage.setItem(sessionUserAttemptsKey, currentAttemptAmount);
        sessionUserAttemptsField.innerText = currentAttemptAmount;

        // Update local maksimum salah saat ini juga agar Local Stats langsung berubah.
        const localMaximumAttempts = parseInt(localStorage.getItem(localMaximumAttemptsKey)) || 0;
        if (currentAttemptAmount > localMaximumAttempts) {
            localStorage.setItem(localMaximumAttemptsKey, currentAttemptAmount);
            localMaximumAttemptField.innerText = currentAttemptAmount;
        }
